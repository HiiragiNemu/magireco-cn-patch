...
class EdgeOneHandler:
    ...
    def purge_cache(self, domain, filename):
        try:
            ...
            logger.log(f"[EdgeOne] 成功提交缓存刷新任务: {target_url}，任务ID: {masked_task_id}", "INFO")
            return True, masked_task_id
        except Exception as e:
            logger.log(f"[EdgeOne] 刷新缓存 {domain}/{filename} 失败: {e}", "ERROR")
            return False, str(e)
...
def main():
    ...
    try:
        ...
        repo_name = os.environ.get('GH_REPO', 'HiiragiNemu/magireco-cn-patch')
        repo_parts = repo_name.split('/')
        repo_owner = repo_parts[0] if len(repo_parts) > 1 else 'HiiragiNemu'
        repo_repo = repo_parts[1] if len(repo_parts) > 1 else 'magireco-cn-patch'
