...
class EdgeOneHandler:
    ...
    def purge_cache(self, domain, filename):
        try:
            ...
            logger.info(f"[EdgeOne] 成功提交缓存刷新任务: {domain}/{filename}, 任务ID: {resp.RequestId}")
        except Exception as e:
            logger.error(f"[EdgeOne] 刷新缓存 {domain}/{filename} 失败: {e}")
            raise
...
def main():
    ...
    gh_token = os.environ.get('GH_TOKEN', '') # 需要在 Secrets 中添加 GitHub Token
