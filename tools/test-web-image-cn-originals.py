...
assert len(m['originalCnImages'])==21
...
assert len(m['userRequestedRestorations'])==1
for row in m['userRequestedRestorations']:
    assert hashlib.sha256((root/row['path']).read_bytes()).hexdigest()==row['sha256'],row['path']
print(json.dumps(dict(status='PASS',exactCnOriginals=21,acceptedImagesUnchanged=11,alreadyCnImagesUnchanged=3,AIRepainting=False,nativeCardsChanged=False,VFXTChanged=False,runningConnectingRestored=1)))
