...
assert len(m['originalCnImages'])==10
...
for row in m['preservedAcceptedImages']:
    assert hashlib.sha256((root/row['path']).read_bytes()).hexdigest()==row['sha256'],row['path']
print(json.dumps(dict(status='PASS',exactCnOriginals=10,acceptedImagesUnchanged=11,AIRepainting=False,nativeCardsChanged=False,VFXTChanged=False)))
