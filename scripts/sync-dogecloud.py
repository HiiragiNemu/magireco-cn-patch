...
不混进 sync-and-upload.yml 的 object-storage 系（object-storage 桶 + edge/ESA/CDN）。
...
本脚本流程：
  ...
  5. 流式上传 to_upload（boto3，virtual hosted style，与 object-storage 同款边下边传）
  6. confirm_cleanup=true 时删除 to_delete，否则仅列出警告（与 object-storage 语义一致）
...
class Doge:
    ...
    def upload_file_from_url(self, url, filename):
        ...
        try:
            ...
            # 与 object-storage 系同款。给源站拉取加 300s read 超时，防止 urlopen 无限阻塞。
...
def main():
    ...
    # 过时文件：默认只列不删，confirm_cleanup=true 才删（与 object-storage 系同语义）
