define("underscore backbone backboneCommon ajaxControl command text!template/shop/ShopTop.html js/view/shop/ShopTopPartsView js/view/shop/ShopPopupView cardUtil memoriaUtil".split(" "), function(f, t, c, u, k, v, p, q, w, x) {
    var g, y = function() {
            ...
            b.id = a ? a : "101799";
            b.x = 150;
            b.y = 1024 === c.displayWidth ? Math.floor(c.doc.getElementsByTagName("body")[0].offsetHeight / 2) :
                Math.ceil(c.shortSize / 2);
            b.type = 0;
        ...
        m = function(a) {
            ...
            switch (a.itemId) {
                case "MONEY":
                    e =
                        "MONEY" !== a.shopType && "MONTHLY" !== a.shopType && "SELECTABLE_UNOWNED_CHARA" !== a.shopType ? c.getTotalStone().totalMoney : c.getTotalStone().userMoney;
                    break;
                case "RICHE":
    ...
    return t.View.extend({
        ...
        createDom: function() {
            ...
            p.prototype.template =
                f.template($("#shopParts").text());
                ...
                if (0 < a.shopItemList.length) {
                    ...
                    f.each(h, function(b) {
                        if ("GEM" === b.shopItemType) {
                            var d =
                                c.storage.userCharaList.findWhere({
                                    charaId: Number(b.genericId)
                                });
                            if (!d) return;
                            1042 === Number(b.genericId) && (b.description = c.storage.user.get("loginName") + "用于魔力解放的道具")
                            ...
                            }) && (b.alreadyGet = !0) : "FORMATION_SHEET" === b.shopItemType ?
                            c.storage.userFormationSheetList.findWhere({
                                formationSheetId: b.formationSheet.id
                            }) && (b.alreadyGet = !0) : "CARD" === b.shopItemType && (1042 === b.chara.id && (b.name = c.storage.user.get("loginName"), "DESTINY_CRYSTAL" === b.needItemId && (b.name = "【期间限定】" + b.name)), "SELECTABLE_UNOWNED_CHARA" === a.shopType && (d = c.storage.userCharaList.findWhere({
                                charaId: b.chara.id
                            }))) && (b.alreadyGet = !0);
                    ...
                    1 > k && (h = c.doc.createElement("p"), h.innerText = "当前没有可购买的商品。", h.className = "noItem", l.appendChild(h));
                    0 < m && (e.buyNum[a.shopId] = m)
                } else h = c.doc.createElement("p"), h.innerText = "当前没有可购买的商品。",
                    h.className = "noItem", l.appendChild(h);
                b.appendChild(n);
                c.doc.getElementById(a.shopId).appendChild(l);
                0 == d && "SELECTABLE_UNOWNED_CHARA" === a.shopType && (d = a.name.replace(/＠/g, ""), h = e.getShopItemNum(a.shopId), d = h.listNum === h.thisHadNum ? d + "　（委托：完成）" : d + ("　（可接受委托：" + (h.listNum - h.thisHadNum) + "/" + h.listNum + "）"), c.doc.getElementById("listTitle").innerHTML = d);
                e.scrollObj.push("type" + a.shopId)
            });
            c.doc.getElementById("btnWrap").getElementsByClassName("btnScrollInner")[0].appendChild(b);
            c.scrollSetX("btnWrap",
                "btnScrollInner");
        ...
        makeTabBtnAndBaseDom: function(a, e) {
            ...
            b.innerHTML = "\x3cspan\x3e" + a.name.replace(/＠/g, "\x3cbr\x3e") + "\x3cdiv class\x3d'recommendMark'\x3e\x3c/div\x3e\x3c/span\x3e";
        ...
        toggleDisp: function(a, e) {
            if (a &&
                (a.preventDefault(), c.isScrolled() || this.displayType === a.target.dataset.shoptype)) return;
            ...
            a = b.name.replace(/＠/g, "");
            "SELECTABLE_UNOWNED_CHARA" === b.shopType && (b = this.getShopItemNum(this.displayType), a = b.listNum === b.thisHadNum ? a + "　（委托：完成）" : a + ("　（可接受委托：" + (b.listNum - b.thisHadNum) + "/" + b.listNum + "）"));
        ...
        conditionCheck: function(a) {
            if (-1 !== this.conditionCheckItemIds.indexOf(a |
                    0)) {
