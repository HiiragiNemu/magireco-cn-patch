define("underscore backbone backboneCommon ajaxControl command js/event/arenaMission/view/ArenaConfirmView".split(" "), function(f, d, b, g, e, c)
{
  return d.View.extend(
  {
    ...
    {
      a.preventDefault();
      b.isScrolled() || (b.doc.getElementById("tapBlock").style.display = "none", c.prototype.parentView = this, a = new c, a.model = this.model, console.log("对战确认model:", a.model), b.arenaConfirmView = a, b.doc.getElementById("overlapContainer").appendChild(a.render().el), e.getBaseData(b.getNativeObj()))
    },
    removeView: function()
    {
      this.off();
