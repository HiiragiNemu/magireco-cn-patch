...
打开 `magireco_v26_translation_review_1565.xlsx`，查看同一行的：
...
工作簿只有 `人工审核1565` 一个工作表。D:N 是随行移动的隐藏绑定，用来保证排序、筛选和保存后仍能按稳定 ID 安全回填；用户无需查看或填写。

## 当前预填状态

- 人工校对范围：1,565 条当前仍属低权重机器来源的文本。
- 其中 29 条已经按本轮明确指令采用经过结构校正的建议中文；工作簿 C 列预填该值。
- 其余 1,536 条 C 列预填当前中文。
- 323 条已由官方国服／Wiki／确认人工解决，以及 24 条已被更高权威遮蔽的历史机器文本，均不出现在工作簿中。
...
导入必须验证 1,565 个稳定 ID、原文哈希、source-record 哈希、target-contract 哈希、隐藏绑定和非空最终值；任一漂移都停止且不写产品树。
