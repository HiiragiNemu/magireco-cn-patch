...
- 当前真正需要人工语义裁决：199 条，全部属于当前低权重文本；中文字段为空的翻译 backlog 为 0。
...
预期状态：封存文件 17/17；总量 1,912；已审 1,472；人工接管 440；Pass20 权威关闭 323；人工决策待办 199；官方产品更正 56 条记录/59 处字面替换；保护变化 0。决策表尚未填写时，校验器应明确报告 `release_gate_open=false`，这属于正确的关闭状态。
...
2. 在“审核”页逐行阅读原文、当前中文、建议中文、理由和实际使用位置。
...
“审核”页会直接显示每条文本的“维护层类型”“维护表”“原文键”“路径前缀”“产品目标路径”“匹配次数”和“上下文片段”。这些字段的含义是：
...
这些信息用于帮助审核者理解译文出现在哪里；审核者仍只裁决中文语义。当前目标清单基线为 **180 条可精确定位运行时文本、19 条仅维护层项目、246 个精确出现位置、0 个目标冲突**。19 条仅维护层项目仍保留人工决定，但在当前产品树中没有满足安全条件的直接写入目标。`LOW-MT-01485` 不在这 199 条中：运行时和 effective 已采用官方“心魔战”，旧低权重历史行仅保留 provenance，不再制造冗余人工候选。

### 3.2 维护代理的确定性回填流程

工作簿本身不直接修改 `magica/`。维护代理接收另存的完成版 XLSX 后，先严格校验 199 个稳定 ID、行数、原文哈希、锁定来源字段及目标清单，再把人工决定导入 i18n 审核/维护层。任何零匹配、目标漂移、非预期多义、字段结构变化或未决决定都会使流程关闭，且不写产品树。199 条最终值全部进入 canonical i18n 的 `human-reviewed` 输入；其中 180 条再按精确目标提升到 `magica/`，19 条当前无安全运行时目标的项目仍永久保存在 canonical i18n，并明确标记“当前产品未引用”。
...
$completedWorkbook = Join-Path $pass20Work 'pass20_human_review.completed.xlsx'
